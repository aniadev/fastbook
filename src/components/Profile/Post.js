...
import Avatar from "../Newfeed/Avatar";

// redux store
import { useSelector } from "react-redux";

function Post({ postData, reactPost, unReactPost, deletePost }) {
  const user = useSelector((state) => state.user);
  // check reactions
  const reactPostHandle = async () => {
    if (postData.likeId) {
      unReactPost(postData.postId);
    } else {
      reactPost(postData.postId);
    }
  };
          ...
          <div className="nf-post--option">
            ...
            <div className="dropdown-menu" aria-labelledby="dropdownPostOption">
              ...
              <button
                ...
                onClick={() => deletePost(postData.postId)}
              >
                Delete
              </button>
            </div>
